...
@contextmanager
def temporary_mgb(filename: str):
...
def preprocess_mgb(cloud_object: CloudObject, threads: int = 1):
    ...
    try:
        with mask.enabled_all(), temporary_mgb(file_name) as (fpath, workdir):
...
class MGBSlice(CloudObjectSlice):
    ...
    def partial_download(self, threads: int = 1):
        ...
        sv = _sv()
        sv["dp_mode"].value = 1
        _fill_ranges_mode1(sv["read_ranges"], entries)

        shims_mapping = ShimsMapping()
        mask = EnabledMask()
        try:
            with mask.enabled_all(), temporary_mgb(file_name) as (fpath, workdir):
                shims_mapping.info = FileInfo(name_c, raw_c, self.cloud_object.size)
